...
/*   Updated: 2020/10/18 13:49:28 by mlebrun          ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void	ft_putnbr_dec_or_hexa(t_format *format_parsed, long int nb)
{
	if (format_parsed->type == 'x')
	...
	else if (format_parsed->width != 0)
	{
		...
		if (format_parsed->zero_flag == 0)
		{
			while (++i < format_parsed->width
				...
				ft_putchar(' ', format_parsed);
			ft_putnbr_dec_or_hexa(format_parsed, nb);
		}
		else
		{
